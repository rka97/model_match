...
def init_plainlm_model():
    ...
    for name, param in model.named_parameters():
        if "embed_tokens" in name:
            print(
                f"  Embedding init: mean={param.mean().item():.6f}, std={param.std().item():.6f}"
            )
        if "lm_head" in name and not model.cfg.tie_embeddings:
            print(
                f"  LM head init: mean={param.mean().item():.6f}, std={param.std().item():.6f}"
            )
...
def compare_predictions(input_sequence, k=5):
    ...
    for step in range(k):
        ...
        with torch.no_grad():
            ...
            torch_top5_probs = torch.softmax(torch_next_token_logits[0], dim=-1)[
                torch_top5_indices
            ]
        ...
        print(
            f"  JAX next token: {jax_next_token[0]}, logit: {jax_next_token_logits[0, jax_next_token[0]]:.2f}"
        )
        print(
            f"  PyTorch next token: {torch_next_token.item()}, logit: {torch_next_token_logits[0, torch_next_token].item():.2f}"
        )
        ...
        for i, (idx, prob) in enumerate(
            zip(torch_top5_indices.numpy(), torch_top5_probs.numpy())
        ):
            print(f"    {i+1}. Token {idx} (prob: {prob:.6f})")

        # Extend inputs with predicted tokens
        extended_jax_input = jnp.concatenate(
            [extended_jax_input, jax_next_token[:, None]], axis=1
        )
        extended_torch_input = torch.cat(
            [extended_torch_input, torch_next_token.unsqueeze(1)], dim=1
        )
    ...
    _, nanodo_predictions = nanodo_model.apply(
        nanodo_params, jax_input, k, method=nanodo_model.predict
    )
    ...
    if not match:
        ...
        matching_tokens = sum(
            n == p for n, p in zip(nanodo_predictions, plainlm_predictions)
        )
        print(
            f"Number of matching tokens: {matching_tokens}/{k} ({matching_tokens/k*100:.1f}%)"
        )
...
def simple_model_test():
    ...
    np_embedding = np.random.RandomState(42).normal(
        0, 0.02, (vocab_size, embedding_dim)
    )
