...
def get_hf_dataloader(batch_size: int = 8, seq_len: int = 32, framework: str = "torch", split = "train"):
    """
    ...
    fw = load_dataset("HuggingFaceFW/fineweb-edu", name="sample-10BT", split=split, streaming=False, cache_dir="/home/ak4605/data/")
    fw = fw.batch(batch_size=batch_size, drop_last_batch=True)
    def batch_iterator():
        for doc in fw:
            if framework == "torch":
                # Tokenize document text
                _tokenize = lambda x: tokenizer(x, return_tensors="pt")["input_ids"].squeeze()
                token_ids = torch.stack([_tokenize(x)[:seq_len+1] for x in doc['text']])
                ...
                _tokenize = lambda x: tokenizer(x, return_tensors="jax")["input_ids"].squeeze()
                token_ids = jnp.stack([_tokenize(x)[:seq_len+1] for x in doc['text']])
...
def test_hf_dataloader(framework: str = "torch"):
    ...
    if framework == "torch":
        ...
        assert inputs.shape == (batch_size, seq_len, vocab_size)
        assert targets.shape == (batch_size, seq_len, vocab_size)
        assert jnp.all(inputs[:, 1:] == targets[:, :-1]), "Targets should be shifted inputs"
