...
from abstract_training_pipeline import AbstractModel, train, Hyperparameters, distributed_data_generator

# Suppress JAX dtype warnings about int64 to int32 conversion
warnings.filterwarnings("ignore", message="Explicitly requested dtype.*truncated to dtype int32")
...
def train_jax_model(
    ...
    for step in range(args.num_iterations + 1):
        ...
        if step % args.val_loss_every == 0 or step == args.num_iterations:
            ...
            val_loss = evaluate_jax_model(state, val_loader, model_apply, batch_size, args.val_tokens)
            
            print(f"[JAX] Step: {step}/{args.num_iterations}, "
    ...
    if use_compile and hasattr(torch, 'compile'):
        ...
        for step in range(args.num_iterations + 1):
            ...
            if step % args.val_loss_every == 0 or step == args.num_iterations:
                ...
                # Run validation using the extracted function
                val_loss = evaluate_pytorch_model(model, val_loader, batch_size, args.val_tokens)
                
                print(f"[PyTorch] Step: {step}/{args.num_iterations}, "
                     f"Val Loss: {val_loss:.4f}, "
                     f"Training time: {total_time*1000:.0f}ms, "
                     f"Ms/step: {total_time*1000/max(step, 1):.2f}")
...
def compare_training(config: Optional[BenchmarkConfig] = None) -> None:
    ...
    if pytorch_times:
        ...
        print(f"PyTorch average ms/step: {avg_pytorch_time*1000/args.num_iterations:.2f}")
