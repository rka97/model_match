...
def compare_implementations(batch_size=2, seq_len=10, dim=256):
    ...
    print(
        f"Comparing RMSNorm implementations with input shape: ({batch_size}, {seq_len}, {dim})"
    )
    ...
    if not is_equivalent:
        ...
        custom_norm_factor = torch.rsqrt(
            torch_input.pow(2).mean(-1, keepdim=True) + custom_norm.eps
        )

        # For PyTorch RMSNorm, we need to analyze how it's implemented internally
        # This is an approximation based on the formula
        pytorch_norm_factor = 1.0 / torch.sqrt(
            torch_input.pow(2).mean(-1, keepdim=True) + pytorch_norm.eps
        )

        norm_factor_diff = (
            torch.abs(custom_norm_factor - pytorch_norm_factor).mean().item()
        )
        ...
        normalized_pytorch = F.rms_norm(
            torch_input, pytorch_norm.normalized_shape, None, pytorch_norm.eps
        )
